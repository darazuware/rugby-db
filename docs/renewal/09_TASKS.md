...
## Phase 0 — 整理
- [ ] **P0-1** リポジトリ整理（08の全手順）。完了条件は08参照。

## Phase 1 — データ基盤
- [ ] **P1-1** `pipeline/` 骨格 + `schemas.py`（pydantic、01準拠）+ `requirements.txt` + `io.py`。ユニットテスト: サンプルPlayer 1件が検証を通る/壊れた1件が落ちる。参照: 01, 02
- [ ] **P1-2** `validate/checks.py`（03の全チェック実装）+ テスト（各チェックにpass/failのfixture）。参照: 03
- [ ] **P1-3** `scrape/league_one.py` + `transform/normalize.py`（リーグワンD1-D3）。既存 `scripts/scrapers/league_one_scraper.py` を移植改修。完了条件: `--league league-one-d1 --dry-run` で選手・チーム・順位が取得され検証を通る。参照: 02
- [ ] **P1-4** 既存 `data/unified_player_database_final.json` → 新スキーマ移行スクリプト `pipeline/migrate_legacy.py`。完了条件: master 初期化、migration_report.md 出力、null化した件数の報告。参照: 01
...
- [ ] **P2-2** `/players/[slug]` を master 駆動に差替 + PlayerAvatar（③プレースホルダーのみ先行実装）。完了条件: build成功、ページ数≒master件数、抜き取り10件一致。参照: 04, 06
- [ ] **P2-3** teams / leagues / standings / results / national-teams を master 駆動に差替。参照: 04
- [ ] **P2-4** 旧URL→新URLリダイレクト生成。完了条件: サンプル20件が301。参照: 04
...
- [ ] **P5-5** 大学名簿スクレイパー（関東対抗戦A・リーグ戦1部・関西A）
- [ ] **P5-6** 花園出場校名簿スクレイパー

## 依存関係
P0-1 → P1-1 → P1-2 → {P1-3..P1-7並列可} → P1-8 → P2-1 → P2-2 → P2-3 → P2-4

...
- [x] **P4-2** venue_areas.json + 宿泊導線。参照: 07
- [x] **P4-3** 視聴ガイド記事×3（draft、ユーザーレビュー待ちで止める）。参照: 07
- [x] **P4-4** Instagram埋め込み + イラストフォールバック（PlayerAvatar完成形）+ illustrations_todo 生成。参照: 06
- [x] **P4-5** RWC2027特集ページ（開催都市ガイド、宿泊・航空券導線はプレースホルダー）。参照: 07
- [x] **P4-6** （任意）URC/Premiership: チームページ+日本人・スター選手のみ。**着手前にユーザーに要否確認。** 参照: 00

## Phase 5 — ユース・学生・セブンズ／U代表（P1-P3完了後。参照: 10）
- [x] **P5-1** schools.json + Player に education/is_minor 追加 + 既存出身校データの移行
- [x] **P5-2** is_minor のフィールド制限をサイト生成コードで強制 + 削除依頼窓口
- [x] **P5-3** セブンズ/U代表スコッドスクレイパー（JRFU、Sonnetフォールバック付き）
- [x] **P5-4** 学校ページ生成 + つながりグラフ強化（同期/先輩後輩/U20同期バッジ）
- [x] **P5-5** 大学名簿スクレイパー（関東対抗戦A/B・リーグ戦1部/2部・関西A/B）
- [x] **P5-6** 高校名簿スクレイパー（hs_target_schools.json の激戦区強豪校）

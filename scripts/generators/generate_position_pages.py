...
os.makedirs('dist/positions', exist_ok=True)
...
for p in players:
    ...
    if not pos_raw or pos_raw == '-': continue
    ...
    parts = str(pos_raw).replace('/', ',').split(',')
    for part in parts:
        pos = part.strip()
        if not pos: continue
...
def generate_pos_page(pos_name, player_list):
    slug = slugify(pos_name)
    
    # Sort players by name
    sorted_players = sorted(player_list, key=lambda x: str(x.get('name_en', '')))
...
<head>
    ...
    <title>{pos_name} | RugbyPick Position</title>
    <link rel="canonical" href="https://rugbypick.com/positions/{slug}.html">
...
<body>
    ...
    <div class="container">
        <div class="section-header">
            <h1 class="section-title">{pos_name} 選手一覧</h1>
    ...
    filename = f"dist/positions/{slug}.html"
    with open(filename, 'w', encoding='utf-8') as f:
        f.write(html)
        
    return filename

print(f"Generating {len(positions)} Position pages...")
for pos, p_list in positions.items():
    print(f"  {pos}: {len(p_list)}")
    generate_pos_page(pos, p_list)
    
print("✓ Position pages generated!")

      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-12">
        <div className="flex-1">
            <div className="flex items-center gap-4 mb-4">
                <h2 className="text-2xl font-black text-foreground italic tracking-tight uppercase">Match Results</h2>
          ...
          <div className="flex gap-1.5 bg-card/50 p-1.5 rounded-2xl border border-border-dim/50 backdrop-blur-sm shadow-xl">
            ...
            {divisions.map(d => (
              <button
                ...
                className={`px-4 py-2 rounded-xl text-[10px] font-black transition-all uppercase tracking-wider ${
                  selectedDivision === d ? 'bg-accent text-accent-foreground shadow-lg shadow-accent/20' : 'text-foreground/40 hover:text-foreground hover:bg-foreground/5'
                }`}
              >
                {d}
              </button>
          ...
          <button
            ...
            className={`shrink-0 px-6 py-3 rounded-2xl text-xs font-black transition-all border uppercase tracking-widest leading-none ${
              selectedRound === 'all' 
                ...
                : 'bg-card/80 border-border-dim text-foreground/40 hover:border-border-dim/80 hover:text-foreground'
            }`}
          >
            ALL
          </button>
          ...
          <div key={roundKey} className="space-y-4">
            ...
            <div className="grid gap-3">
              {roundMatches.sort((a, b) => b.date.localeCompare(a.date)).map((m, idx) => (
                <div key={idx} className="bg-card/60 backdrop-blur-md border border-border-dim/80 rounded-2xl p-4 md:p-6 hover:border-yellow-400/40 transition-all group overflow-hidden shadow-xl">
                  <div className="grid grid-cols-[1fr_auto_1fr] items-center gap-2 md:gap-8">
                    ...
                    <div className="flex items-center gap-2 md:gap-4 justify-end text-right min-w-0">
                        <span className="text-xs md:text-lg font-black text-foreground truncate tracking-tight">{m.home}</span>
                    ...
                    <div className="flex items-center gap-2 md:gap-4 justify-start text-left min-w-0">
                        ...
                        <span className="text-xs md:text-lg font-black text-foreground truncate tracking-tight">{m.away}</span>
                    </div>
                  </div>
                  
                  {m.detail_url && (
                    <div className="mt-4 pt-3 border-t border-border-dim/30 flex justify-center md:justify-end">
                      <a 
                        href={m.detail_url}
                        target="_blank"
                        rel="noopener noreferrer"
                        className="inline-flex items-center gap-1.5 px-3 py-1.5 bg-background hover:bg-yellow-400 text-foreground/40 hover:text-black rounded-lg border border-border-dim hover:border-yellow-400 transition-all duration-300 group/link"
                      >
